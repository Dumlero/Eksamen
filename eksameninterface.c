...
int main (int argc, char const *argv[]) {
   ...
   printf("\nScanned succesfully from the 2015-2016 data if %s == Mandag\n\n",
 ...
 if(argc > 1 && strcmp(argv[1], "--print") == 0){
   ...
   char daarlige_fans[]="Får inputtet, hvem der har faerrest tilskuere i 2015?";
...
void gem_information(char *filename, KAMP *INFORMATION, int linietal){
  ...
  for (i = 0; i < linietal;i++) {
    ...
    if(res != 12){
      ...
      /* fordi dotten ødelægger input bliver der rundet til 1000'ende ud fra de 2 første tal */
...
int hold_sammenlign(const void *a, const void *b){
  ...
  if (HOLDA->point == HOLDB->point &&
     (HOLDA->maal - HOLDB->maalimod) == (HOLDB->maal - HOLDB->maalimod) &&
      HOLDA->maal == HOLDB->maal) {
    return strcmp(HOLDA->holdnavn, HOLDB->holdnavn);

  }else if(HOLDA->point == HOLDB->point &&
     (HOLDA->maal - HOLDB->maalimod) == (HOLDB->maal - HOLDB->maalimod)){
    return HOLDA->maal - HOLDB->maalimod;

  }else if(HOLDA->point == HOLDB->point){
    return (HOLDA->maal - HOLDB->maalimod) == (HOLDB->maal - HOLDB->maalimod);
